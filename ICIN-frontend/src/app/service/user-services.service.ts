...
export class UserServicesService {
  ...
  addUSerToDb(){
    ...
    //this.http.post("http://localhost:3309/register",user);
    //return this.http.get("http://localhost:3309/hello",{responseType: 'text'})
    return this.http.post("http://localhost:3309/register",
  ...
  authenticateUser(user:User){
      ...
      return this.http.post("http://localhost:3309/login",user);
  }
  isUserLogin(){
  ...
  transferFund(obj:Transaction){
    return this.http.post("http://localhost:3309/transfer",obj);
  }

  getAccountBalance(accountId){
    return this.http.get(`http://localhost:3309//accounts/${accountId}/balance`)
  }
  ...
  depositMoney( accType:String, accNo:number, amount:number){
    ...
    return this.http.get(`http://localhost:3309/deposit/${accType}/${accNo}/${amount}`);
  }
  ...
  requestChequeBook(chequeObj:ChequeBook,userObj:User){
    ...
    return this.http.post("http://localhost:3309//createcheque",chequeObj);
  }

  withdrawMoney(accType:String, accNo:number, amount:number){
    return this.http.get(`http://localhost:3309/withdraw/${accType}/${accNo}/${amount}`);
  }

  getTransactions(accountId){
    console.log(accountId);
    return this.http.get(`http://localhost:3309//accounts/${accountId}/transactions`)
  }
}
